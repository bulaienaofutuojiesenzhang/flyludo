...
export default class SmsCodeButton extends React.Component {
	constructor(props) {
	  super(props)
		this.state = {
	...
	render(){
		...
		return (
			<TouchableOpacity activeOpacity={counting ? 1 : 0.8} onPress={()=>{
				...
				<View style={[{ paddingLeft: 15, paddingRight: 15, height:38,justifyContent:'center',alignItems:'center',backgroundColor: ((!counting && enable && selfEnable) ? ('#8CCBFE') : '#efefef'),borderRadius:25},style]}>
					<Text style={[{fontSize: 14,fontWeight:'400'},textStyle,{color: ((!counting && enable && selfEnable) ? (textStyle ? textStyle.color : 'blue') : disableColor || 'gray')}]}>{timerTitle}</Text>
				</View>
			</TouchableOpacity>
		)
	}
