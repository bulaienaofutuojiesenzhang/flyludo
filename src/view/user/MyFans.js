...
class MyFans extends Component {
  ...
  initFunc() {
    this.setState({ isLoading: true, showFoot: 0, page: 1 }, () => {
      Http('get', `/api/artist/fans/list/${this.state.page}/${10}`, {
  ...
  _onEndReached() {
    ...
    Http('get', `/api/artist/fans/list/${page}/${10}`, {
    ...
    return (
      <View style={Styles.followerItem}>
        ...
        {/* <TouchableOpacity
          style={[
            Styles.followButton,
            { backgroundColor: !item.mutual ? '#ddd' : '#f44336' },
          ]}
          onPress={() => this.handleFollow(item.id)}
        >
          <Text style={[Styles.followButtonText, { color: !item.mutual ? '#555' : '#fff' }]}>
            {!item.mutual ? '互相关注' : '回关'}
          </Text>
        </TouchableOpacity> */}
