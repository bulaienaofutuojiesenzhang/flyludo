...
class ApproveUser extends Component {
  ...
  handleSubmit = () => {
    ...
    let params = {
      // "idcard": idNumber,
      "name": name,
      "certificateType": 'identification_card',
      // "idcardStart": Moment(this.state.startDate).format('YYYY-MM-DD'),
      "certificateImgUrl": frontImage,
      "certificateImgUrl2": backImage,
      // "certificateImgUrl2": idcardHoldUrl,
      // "city": [
    ...
    if (this.state.selectedType == 0) {
      params.idcardEnd = Moment(this.state.endDate).format('YYYY-MM-DD')
    }
  ...
  render() {
    ...
    return (
      <ScrollView style={Styles.container}>
        ...
        {/* 输入区域 */}
        {
          this.state.renzhengState && this.state.renzhengObj.certificateType == 'identification_card' ?
          ...
          {/* <View style={Styles.itemView}>
            <Text style={Styles.leftText}>证件类型</Text>
            <TouchableOpacity 
            ...
            >
              <Text style={[Styles.pickerText, !this.state.selectedType && Styles.placeholderText]}>
                {this.state.selectedType ? 
                  ["身份证", "护照", "港澳通行证"][parseInt(this.state.selectedType) - 1] : 
                  "请选择证件类型"}
              </Text>
            </TouchableOpacity>
            <BottomPicker
          ...
          {/* {
            this.state.selectedType == 1 ?
              <View style={{ paddingBottom: 8 }}>
                <Pressable onPress={() => this.setState({ dateOpen: true, changeDate: 'end' })} style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', height: 60, paddingBottom: 10 }}>
                  {
                    this.state.endDate ?
                      <Text style={{ color: Colors.hei2E }} >{Moment(this.state.endDate).format('YYYY-MM-DD')}</Text>
                      :
                      <Text style={{ color: Colors.hui66 }} >请输入身份证结束时间</Text>
                  }
        ...
        {/* 图片上传区域 */}
        <View style={Styles.uploadContainer}>
          <Text style={Styles.uploadTitle}>上传身份证认证信息</Text>
          <View style={Styles.imageRow}>
            ...
            >
              {backImage ? (
                ...
                <Image source={require('../../asserts/images/publish/icon_idcard_reverse.png')} style={Styles.uploadedImage} />
              )}
            </TouchableOpacity>
            ...
            >
              {idcardHoldUrl ? (
                ...
                <Image source={require('../../asserts/images/publish/icon_idcard_hand.png')} style={Styles.uploadedImage} />
              )}
            </TouchableOpacity> */}
        ...
        <View style={Styles.instructions}>
          <Text style={Styles.instructionsText}>上传要求:</Text>
          <Text style={Styles.instructionsText}>
            1. 请上传本人身份证正反面照片
          </Text>
          <Text style={Styles.instructionsText}>
            2. 拍照时对焦点请置于身份证上，保证身份证信息清晰且未遮挡
          </Text>
          <Text style={Styles.instructionsText}>
            3. 上传文件格式支持png、jpg和bmp
          </Text>
          <Text style={Styles.instructionsText}>
            4. 文件大小不超过3MB
          </Text>
          <Text style={Styles.instructionsText}>
            5. 同城有约会妥善保管您上传的信息，不会将其用于商业用途或泄露给第三方
          </Text>
          <Text style={Styles.instructionsText}>
            6. 如果审核未能通过，您可以联系客服查看原因
          </Text>
